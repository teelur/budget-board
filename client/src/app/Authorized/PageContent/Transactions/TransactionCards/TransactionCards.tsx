...
import {
  getTransactionDisplayItems,
  TransactionDisplayItem,
} from "~/helpers/transactions";
import TransactionTransferGroup from "./TransactionTransferGroup";
...
const TransactionCards = (props: TransactionCardsProps): React.ReactNode => {
  ...
  const displayItems = React.useMemo(
    () => getTransactionDisplayItems(props.currentViewTransactions),
    [props.currentViewTransactions],
  );
  const virtualizer = useVirtualizer({
    ...
    getItemKey: (index) => {
      ...
      return item
        ? `${item.type}-${item.transactions.map((transaction) => transaction.id).join("-")}`
        : index;
    },
    overscan: 5,
  });
          ...
          {hasTransactions ? (
            ...
            >
              {virtualizer.getVirtualItems().map((virtualItem) => {
                const displayItem: TransactionDisplayItem | undefined =
                  displayItems[virtualItem.index];

                if (displayItem === undefined) {
                  return null;
                }

                const transaction = displayItem.transactions[0];
                ...
                return (
                  ...
                  >
                    {displayItem.type === "transfer" ? (
                      <TransactionTransferGroup
                        transactions={displayItem.transactions}
